...
export default function Button({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`flex min-h-10 items-center justify-center gap-1 rounded-4xl px-4 py-2 font-bold transition-all ease-in-out disabled:cursor-not-allowed disabled:opacity-50 ${fullWidth ? "w-full" : "w-auto"} ${variantStyles[variant]}`}
    >
      {btnText}
    </button>
  )
